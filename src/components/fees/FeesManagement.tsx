...
export const FeesManagement = () => {
  ...
  const [applyScholarshipModalOpen, setApplyScholarshipModalOpen] = useState(false);
      ...
      try {
        ...
        if (data && Array.isArray(data)) {
          const mapped = data.map((fs: any) => {
            ...
            return { ...fs, className: cls ? `${cls.grade} ${cls.section}` : 'Unknown' };
          });
  ...
  const fetchFeeRecords = async (page: number = 1) => {
    ...
    try {
      const filters: SearchFilters = {
        ...
        scholarship: selectedScholarship !== 'all' ? selectedScholarship : undefined,
      ...
      const mappedFees: FeeRecord[] = res.fees.map((fee: any) => ({
        ...
        scholarshipType: fee.scholarships.length
          ...
          : 'None',
        nextDueDate: fee.installments?.find((inst: any) => inst.status !== 'Paid')?.dueDate,
  ...
  const lateSubmitters = feeRecords.filter(r => r.dueAmount && r.status !== 'Paid');
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {isAdmin && (
            <>
              ...
              <Button onClick={() => setViewFeeStructureModalOpen(true)} variant="outline">
              ...
              <TableBody>
                {feeRecords.map(record => (
                  <TableRow key={record.id}>
                    ...
                    <TableCell>
                      {record.nextDueDate ? new Date(record.nextDueDate).toLocaleDateString() : 'Not set'}
                    </TableCell>
                    <TableCell>
      ...
      {applyScholarshipModalOpen && (
        <ApplyScholarshipModal isOpen={applyScholarshipModalOpen} onClose={() => setApplyScholarshipModalOpen(false)} />
      )}
      {collectFeeModalOpen && (
        <CollectFeeModal isOpen={collectFeeModalOpen} onClose={() => setCollectFeeModalOpen(false)} students={students} />
      )}
      {feeStructureModalOpen && <FeeStructureModal isOpen={feeStructureModalOpen} onClose={() => setFeeStructureModalOpen(false)} />}
      <ClassFeeStructureModal isOpen={classFeeStructureModalOpen} onClose={() => setClassFeeStructureModalOpen(false)} classes={classes} />
      <ViewFeeStructure isOpen={viewFeeStructureModalOpen} onClose={() => setViewFeeStructureModalOpen(false)} structures={feeStructures} />
