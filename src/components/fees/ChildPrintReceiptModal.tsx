 ...
 * - isOpen: boolean
 * - onClose: () => void
 * - payment: any (the payment object you pass from FeesManagement)
 * - studentName: string
 * - className: string
 *
 * Notes:
 * - Exports a named component `ChildPrintReceiptModal` (so your import { ChildPrintReceiptModal } works).
 * - Renders two receipt blocks (ORIGINAL and COPY) with a small difference (COPY has faded watermark text).
 * - Uses window.print() to print the modal contents. We render printable content inside a dedicated container
 *   and temporarily open the print dialog when Print is clicked.
 */
...
export const ChildPrintReceiptModal = ({
  ...
  const paidAt = payment?.paidAt ?? payment?.paidAtDate ?? payment?.date ?? null;
  ...
  const handlePrint = () => {
    ...
    const popup = window.open("", "_blank", "width=800,height=1100,scrollbars=yes");
    if (!popup) {
    ...
    popup.document.open();
    popup.document.write(`<html><head><title>Receipt</title>${styles}</head><body>${printContents}</body></html>`);
  ...
  const PrintableContent = () => (
    <div ref={printRef as any}>
      ...
      <div className="receipt" style={{ position: "relative" }}>
        ...
        <div className="row"><div>Student</div><div>{studentName}</div></div>
        <div className="row"><div>Class</div><div>{className}</div></div>
        <div className="row"><div>Payment For</div><div>{month || "Tuition"}</div></div>
        <div className="row"><div>Amount</div><div>{fmt(paidAmount)}</div></div>
        <div className="row"><div>Mode</div><div>{mode}</div></div>
        <div className="row"><div>Transaction ID</div><div>{txn}</div></div>
        <div className="row"><div>Date</div><div>{paidAt ? new Date(paidAt).toLocaleString() : new Date().toLocaleString()}</div></div>

        <div className="total">Received Amount: {fmt(paidAmount)}</div>
        <div className="copy-note">This is the ORIGINAL receipt. Keep for your records.</div>
      </div>
      ...
      <div className="receipt" style={{ position: "relative" }}>
        ...
        <div className="total">Received Amount: {fmt(paidAmount)}</div>
        <div className="copy-note">This is the CUSTOMER COPY. Please retain for your reference.</div>
      </div>
    </div>
  );
        ...
        <div className="space-y-4">
          ...
          <div style={{ border: "1px solid #eee", padding: 12, borderRadius: 6 }}>
            ...
            <div style={{ marginTop: 8 }}>
              <div style={{ fontSize: 14, fontWeight: 600 }}>{studentName} — {className}</div>
              <div style={{ fontSize: 13, color: "#555", marginTop: 6 }}>
                {month ? `For: ${month}` : "Tuition / Fees"}
              </div>
              <div style={{ marginTop: 8 }}>
                <div style={{ display: "flex", justifyContent: "space-between" }}><span>Amount</span><span>{fmt(paidAmount)}</span></div>
                <div style={{ display: "flex", justifyContent: "space-between" }}><span>Mode</span><span>{mode}</span></div>
                <div style={{ display: "flex", justifyContent: "space-between" }}><span>Txn ID</span><span>{txn}</span></div>
              </div>
            </div>
          </div>
